...
class OrderView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not request.user.is_authenticated:
            return JsonResponse({'Status': False, 'Error': 'Требуется авторизация'}, status=status.HTTP_403_FORBIDDEN)
...
class BasketView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        """создает корзину и добавляет в неё информацию о товарах """
        if not request.user.is_authenticated:
            return JsonResponse({'Status': False, 'Error': 'Требуется авторизация'}, status=status.HTTP_403_FORBIDDEN)
    ...
    def delete(self, request, *args, **kwargs):
        """удаляет информацию о товаре в корзине"""
        if not request.user.is_authenticated:
            return JsonResponse({'Status': False, 'Error': 'Требуется авторизация'}, status=status.HTTP_403_FORBIDDEN)
